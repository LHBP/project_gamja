...
class joinWindow(QWidget,UI_class1) :
    def __init__(self) :
        super().__init__()
        self.setupUi(self)
        self.setWindowIcon(QtGui.QIcon('D:\python\knu\Team\감자그림\감자1.png'))
...
class loginWindow(QWidget,UI_class2) :
    ...
    def button_join_clicked(self) :
        self.hide() # 로그인창 숨김
        self.myWin = joinWindow() # myWin을 loginWindow로 변경
        self.myWin.show() # 메인창 띄움
...
class MainWindow(QWidget,UI_class3) :
    ...
    def __init__(self) :
        ...
        self.btnOk.clicked.connect(self.total_mileage)
    ...
    def total_mileage(self):
        if os.path.isfile('마일리지.csv'):
            print('파일존재')
            df = pd.read_csv('마일리지.csv', encoding='CP949')            
            df = pd.DataFrame([[W_mileage, G_mileage, E_mileage, C_mileage, R_mileage]], columns=['W_mileage', 'G_mileage', 'E_mileage', 'C_mileage', 'R_mileage'])
            
            df.to_csv("마일리지.csv", encoding="cp949", mode='a', header=False, index = False)
            self.labelA.setText('저장 완료!')
        else:
            df = pd.DataFrame([[W_mileage, G_mileage, E_mileage, C_mileage, R_mileage]], 
                        columns=['W_mileage', 'G_mileage', 'E_mileage', 'C_mileage', 'R_mileage'])
            
            df.to_csv("마일리지.csv", encoding="cp949", index = False)
            self.labelA.setText('저장 완료!')
        
        
    def shop(self) :
        self.web.open('https://na9595.github.io/finalproject/project-20220818T024714Z-001/project/main.html')
...
f = open("회원정보.csv", mode="wt", encoding="cp949", newline="")
writer = csv.writer(f)
...
df = pd.read_csv('마일리지.csv', encoding='CP949')
df.DataFrame([{'마일리지':W_mileage}], columns='마일리지')
f = open("마일리지.csv", mode="r", encoding="cp949")
f = open("회원정보.csv", mode="r", encoding="cp949")
reader = csv.reader(f)
