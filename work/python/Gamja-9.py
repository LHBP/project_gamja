...
'''UI_class1 = uic.loadUiType('C:\python\knu\work\python\join.ui')[0]
UI_class2 = uic.loadUiType('C:\python\knu\work\python\login.ui')[0]
UI_class3 = uic.loadUiType('C:\python\knu\work\python\Main_window.ui')[0]'''
# 회원가입창
class joinWindow(QWidget,UI_class1) :
    def __init__(self) :
        super().__init__()
        self.setupUi(self)
        QtGui.QPixmap('C:\python\knu\work\python\로그인, 회원가입 그림\회원가입.png').scaled(self.width()-300, self.height()-300)
...
class MainWindow(QWidget,UI_class3) :
    ...
    def __init__(self) :
        ...
        #등급별 그림
        self.label_imgA.setPixmap(QtGui.QPixmap('C:\python\knu\work\python\level1.png').scaled(self.width()-300, self.height()-300)) #사진링크 바꾸기
    ...
    def level(self) :
        if mileage >= 300 and mileage < 400 :
            ...
            self.label_imgA.setPixmap(QtGui.QPixmap('C:\python\knu\work\python\level2.png').scaled(self.width()-300, self.height()-300)) #사진링크 바꾸기
        elif mileage >= 500 and mileage < 600 :
            self.labelC.setText('등급 : 플레티넘')
            self.label_imgA.setPixmap(QtGui.QPixmap('C:\python\knu\work\python\level3.png').scaled(self.width()-300, self.height()-300)) #사진링크 바꾸기
        elif mileage >= 600 :
            self.labelC.setText('등급 : 다이아몬드')
            self.label_imgA.setPixmap(QtGui.QPixmap('C:\python\knu\work\python\level4.png').scaled(self.width()-300, self.height()-300)) #사진링크 바꾸기
